...
@WebServlet("/AdminSellersViewServlet")
public class AdminSellersViewServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        Object username = session.getAttribute("username"); 
        session.setAttribute("username",username);
		EditSellerDao editSellerDao = new EditSellerDao();
        ...
        out.println("<h1  class='register' >Sellers List</h1>");          
        List<ClientBean> list = editSellerDao.getAllSellers();  
        out.print("<table border='1' width='100%'");  
        out.print("<tr><th>Username</th><th>Name</th><th>Surname</th><th>Password</th><th>Property</th><th>Edit</th></tr>");  
        for(ClientBean e:list) {
        	out.print("<tr><td>"+e.getUsername()+"</td><td>"+e.getName()+"</td><td>"+e.getSurname()+"</td><td>"+e.getPassword()+"</td><td class='center'>"+e.getProperty()+"</td><td class='center'><a class='center' href='AdminEditSellerServlet?username="+e.getUsername()+"'> <img class='image' src='icons/edit.png'></a></td></tr>");
        }
        out.print("</table>");
        out.print("<br>");
        out.print("</div>");
