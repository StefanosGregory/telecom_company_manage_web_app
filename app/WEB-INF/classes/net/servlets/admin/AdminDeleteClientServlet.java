...
@WebServlet("/AdminDeleteClientServlet")
public class AdminDeleteClientServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.setContentType("text/html");  
	        ...
	        if(status>0){  
	            response.sendRedirect("AdminClientsViewDeleteServlet");  
	        }else{  
	        	out.print("<div class='alert'>");
	  	        out.print("<form action='AdminClientsViewDeleteServlet'>Error, try again later!<input type='submit' class='closebtn' value='X'/></form>");  
	        	out.print("</div>");
	        }  
